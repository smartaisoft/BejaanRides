...
const DriverProfile = () => {
  ...
  useEffect(() => {
    console.log('data in redux :', user);
    const fetchUserSubscriptions = async () => {
      if (!user?.uid || userSubscriptions.length > 0) return;
      setLoadingSubs(true);
      try {
        ...
        dispatch(setUserSubscriptions(subsData));
      } catch (error) {
  ...
  }, [user?.uid, userSubscriptions.length, dispatch, user]);
...
const styles = StyleSheet.create({
  ...
  statLabel: {fontSize: 14, color: '#6A1B9A', fontWeight: '600'},
  card: {
  ...
  subscriptionItem: {marginBottom: 10},
});
